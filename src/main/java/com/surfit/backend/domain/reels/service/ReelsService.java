...
public class ReelsService {
    ...
    public ReelsFeedResponse getFeed(Long memberId, Long cursor, int size) {
        ...
        List<Long> viewedPostIds = viewLogRepository.findByMemberId(memberId)
                .stream()
                .map(ViewLog::getPostId)
                .toList();

        List<Post> posts = postRepository.findUnviewedPostsByCategoryIds(
                categoryIds,
                viewedPostIds.isEmpty() ? List.of(-1L) : viewedPostIds,
                cursor,
                size + 1
        );
        ...
        List<ReelsItemResponse> items = result.stream()
                .map(post -> {
